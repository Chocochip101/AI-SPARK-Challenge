...
if(video_name not in videoDict):
    ...
    datasetsH5 = h5py.File("datasets"+os.sep+"object_features"+os.sep+h5Dataset, 'r')
    # datasetsH5 = h5py.File("datasets"+os.sep+"object_features"+os.sep+"eccv16_dataset_summe_google_pool5.h5", 'r')
    seq1 = datasetsH5[videoDict[video_name]][...]
    # seq1 = datasetsH5[videoDict[video_name]]['features'][...] 
    # (636, 1024)  
    print("reshape prior:", seq1.shape)
    seq1 = seq1.reshape((-1, 1024))
    print("shape: ", seq1.shape)
    # seq2 = np.load("datasets"+os.sep+"kinetic_features"+os.sep+dataset+os.sep+"FLOW"+os.sep+"features"+os.sep+video_name+".npy")  # you can provide features from another video
    # seq3 = np.load("datasets"+os.sep+"kinetic_features"+os.sep+dataset+os.sep+"RGB"+os.sep+"features"+os.sep+video_name+".npy")
    
    # minShape=np.min([seq1.shape[0],seq2.shape[0],seq3.shape[0]])
    minShape=np.min([seq1.shape[0]])
    seq1 = cv2.resize(np.float32(seq1), (seq1.shape[1],minShape), interpolation = cv2.INTER_AREA)
    ...
    y, _ = model([seq1],seq_len)
    yArray = y.detach().numpy()[0]
    yArray= getSmoothOutput(yArray)
    # print("output y_hat: ", y)
    print("output yArray: ", yArray)
    print("output yArray Shape: ", len(yArray))
    print(yArray[0])
    temp = []
    for i in range(0, len(list(yArray)) - 1, 2):
      temp.append((yArray[i] + yArray[i + 1]) / 2)
    yArray = temp
    print("output yArray: ", yArray)
    print("output yArray Shape: ", len(yArray))
    np.save('./yResults/' + video_name, yArray)
    xLab = "video time lime"
    yLab = "score to be in video summary"
    tittle = "prediction_score"
    plt.xlabel(xLab)
    plt.ylabel(yLab)
    plt.title(tittle)
    plt.grid(True)
    plt.plot(yArray)
    if(saveImg):
        plt.savefig("media"+os.sep + h5Dataset + os.sep+tittle+video_name+'.png',bbox_inches='tight', dpi=dpi)
